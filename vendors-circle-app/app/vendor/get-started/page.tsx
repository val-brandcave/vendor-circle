...
export default function VendorGetStartedPage() {
  ...
  const hasShownBankModal = useRef(false);
  ...
  // Check for bank connection success (new users from bank invite)
  useEffect(() => {
    if (hasShownBankModal.current) return;
    ...
    if (authMessage) {
      try {
        ...
        if (message.type === 'bank_connected') {
          hasShownBankModal.current = true;
          setConnectedBankId(message.bankId || '');
          setConnectedBankName(message.bankName);
          
          // If first-time modal is showing, wait for it to close
          // Otherwise show immediately
          if (showFirstTimeModal) {
            console.log('Will show bank modal after first-time modal closes');
          } else {
            setTimeout(() => {
  ...
  // Handler for first-time modal close
  const handleFirstTimeModalClose = () => {
    setShowFirstTimeModal(false);
    
    // Check if we need to show bank modal after first-time modal
    const authMessage = sessionStorage.getItem('auth_success_message');
    if (authMessage) {
      try {
        const message = JSON.parse(authMessage);
        if (message.type === 'bank_connected') {
          // Show bank modal after first-time modal closes
          setTimeout(() => {
            setShowBankModal(true);
            sessionStorage.removeItem('auth_success_message');
          }, 500);
  ...
  return (
    <>
      ...
      {/* Bank connected modal (shows after first-time modal for new users) */}
      <BankConnectedModal
